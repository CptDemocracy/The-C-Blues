...
int VectorClear(struct Vector* self) {
    if (!self) {
        errno = EINVAL;
        return EINVAL;
    }
    memset(self->_items, 0, self->_itemCount);
    self->_itemCount = 0;
    return 0;
}

int VectorToArray(struct Vector* self, void* OutArray, size_t OutArrayLength) {
    if (!self || !OutArray || OutArrayLength < 1) {
        errno = EINVAL;
        return EINVAL;
    }
    size_t count = OutArrayLength > self->_itemCount ? self->_itemCount : OutArrayLength;
    for (size_t i = 0; i < count; ++i) {
        memcpy((char*)OutArray + i * self->_itemSize, 
               (char*)self->_items + i * self->_itemSize, 
               self->_itemSize);
    }
    return 0;
}
