...
int StackDuplicate(struct Stack* self, struct Stack* OutResult) {
    ...
    char* item = 0;
    int count = StackGetCount(self);
    for (int i = 0; i < count; ++i) {
        item = (char*)self->_items + i * self->_itemSize;
        StackPush(OutResult, item);
    }
    return 0;
}
...
int StackSwapTopTwoElems(struct Stack* self) {
    ...
    void* top = calloc(1, self->_itemSize);
    if (!top) {
        errno = ENOMEM;
        return ENOMEM;
    }
    void* second = calloc(1, self->_itemSize);
    if (!second) {
        free(top);
        errno = ENOMEM;
        return ENOMEM;
    }
    // important that we make sure we allocated new memory
    // successfully before proceeding
    StackPop(self, top);
    StackPop(self, second);
    StackPush(self, top);
    StackPush(self, second);
    free(top);
    free(second);
    return 0;
}
